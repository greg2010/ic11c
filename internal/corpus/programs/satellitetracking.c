...
// button, so this is the one program in the corpus whose main can return. The
// program stops for good when it does: nothing re-enters main each tick.
//
// SignalStrength is not a strength. A dish answers the degree offset of its
// strongest contact, so a smaller number is a better aim, which is why the
// walks below reverse direction when the reading grows. It answers -1 where
// there is no contact at all and also while one is still resolving, and the
// tests against -1 are what keep those two out of the comparisons.
//
// Pin 3 is deliberately unwired: the lamp is on d4, which is what the housing
// this was written for had, and nothing here reaches d3.
...
void main(void) {
    ...
    while (__ic_load(sw, On) == 1) {
        ...
        __ic_store(dishMed, Setting, 7500);
        __ic_sleep(4);
