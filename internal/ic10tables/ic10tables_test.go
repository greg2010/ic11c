...
// tables is what analysis is handed in production, held through the interface
// it is handed through: binding the two is the whole reason the package exists,
// so a method that stopped satisfying it would not compile here.
var tables sema.Tables = ic10tables.Tables{}
...
// shipped resolves a prefab the build has to ship for the tests below to mean
// anything.
func shipped(t *testing.T, name string) sema.Prefab {
	t.Helper()
	p, ok := tables.PrefabNamed(name)
	if !ok {
...
// logicType resolves a property name to the encoding the refusal queries take.
func logicType(t *testing.T, name string) int {
	t.Helper()
	member, ok := tables.LogicType(name)
	if !ok {
...
func slotType(t *testing.T, name string) int {
	t.Helper()
	member, ok := tables.LogicSlotType(name)
	if !ok {
...
func TestRefusesLogic(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.prefab+" "+tt.property+" "+directionName(tt.dir), func(t *testing.T) {
			if got := shipped(t, tt.prefab).RefusesLogic(logicType(t, tt.property), tt.dir); got != tt.want {
				t.Errorf("RefusesLogic(%s, %s) = %v, want %v", tt.property, directionName(tt.dir), got, tt.want)
			}
		})
	}
}
...
func TestRefusesSlot(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := prefab.RefusesSlot(tt.slot, slotType(t, tt.property), tt.dir); got != tt.want {
				t.Errorf("RefusesSlot(%d, %s, %s) = %v, want %v", tt.slot, tt.property, directionName(tt.dir), got, tt.want)
			}
		})
	}
}
...
func TestNumModes(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.prefab, func(t *testing.T) {
			count, known := shipped(t, tt.prefab).NumModes()
...
func TestHoldsCircuit(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.prefab, func(t *testing.T) {
			holds, known := shipped(t, tt.prefab).HoldsCircuit()
...
func TestPrefabLookup(t *testing.T) {
	...
	byHash, ok := tables.Prefab(hashOf(t, readOnly))
	if !ok {
	...
	if _, ok := tables.PrefabNamed(notShipped); ok {
		t.Errorf("this game build resolves %q, which the misspelling cases elsewhere assume it does not", notShipped)
	}
}

// hashOf answers the number the roster holds one prefab under. Reading it from
...
func hashOf(t *testing.T, name string) int32 {
	t.Helper()
...
func TestOperandNames(t *testing.T) {
	tests := []struct {
		...
		{name: "a logic type", resolve: tables.LogicType, spelling: "Pressure", wantOK: true},
		{name: "a retired logic type", resolve: tables.LogicType, spelling: "ImportQuantity", wantOK: true, wantDeprecated: true},
		{name: "a logic type in the wrong case", resolve: tables.LogicType, spelling: "pressure"},
		{name: "a slot property", resolve: tables.LogicSlotType, spelling: "Occupied", wantOK: true},
		{name: "no slot property", resolve: tables.LogicSlotType, spelling: "NotASlotProperty"},
		{name: "a batch mode", resolve: tables.BatchMode, spelling: "Average", wantOK: true},
		{name: "no batch mode", resolve: tables.BatchMode, spelling: "Median"},
		{name: "a reagent mode", resolve: tables.ReagentMode, spelling: "Contents", wantOK: true},
		{name: "no reagent mode", resolve: tables.ReagentMode, spelling: "Leftovers"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
func directionName(dir sema.Direction) string {
	if dir == sema.Writing {
		return "writing"
	}
	return "reading"
