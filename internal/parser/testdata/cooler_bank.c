...
//
// The bank average is a batch read, which answers NaN when the network holds no
// cooler, and the long long it is narrowed to carries that NaN into the comparison
...
void main(void) {
    ...
    while (true) {
        long long bank = (long long)__ic_load_batch(coolers, Temperature, Average);
