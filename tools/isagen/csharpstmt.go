...
// cutTopOperator splits an expression around the first occurrence of a
// binary operator at bracket depth zero. An operator is only recognized
// where it is not part of a longer one, so the equality in `a >= b` is not
// read as the `>` of a relation.
