...
type assetPrefab struct {
	...
	// UsedPower is absent on a prefab that is not a device.
...
// produces the artifact's prefab table.
//
// The two halves answer different questions and neither is sufficient alone:
// the assets say which things exist and what state each was authored with, and
// the assembly says what a class does with that state.
...
// checkBaseSurface holds that class's own four bodies to still being readable.
//
// The floors in validateDevices catch a roster whose surfaces collapsed to
// nothing. They do not catch the likelier shape of the same failure. This one
// class declaring a body in a spelling the matchers no longer reach leaves every
// class that overrides that body deciding exactly as before, which clears those
// floors comfortably, and leaves the rest of the roster undecided -- which a
// consumer reads as no diagnostic at all rather than as a refusal, so nothing
// downstream reports it either.
//
// What it asks is derived rather than named: with every serialized flag the
// class itself consults set, and a slot of every class declared, each of the
// four bodies has to grant something. An unread body grants nothing whatever the
// game build, and a build where the base genuinely stopped exposing anything
// through one of the four is a change worth a human either way.
...
// serializedStateRE matches the shape the serialized flags are spelled in. The
// evaluator answers a flag missing from a device's state with neither yes nor
// no, so a probe naming them from a list of its own would be refused by the very
// build that renamed one.
var serializedStateRE = regexp.MustCompile(`\bHas\w+State\b`)
...
// for.
//
// C# enums alias freely -- None and Default on zero is the commonplace -- and
// two names on one ordinal leave the inversion nothing to choose between: the
// roster would carry whichever the file declares last, while the bodies compare
// a slot's class against the name they were written with. A slot named by the
// other one matches nothing and reads as refusing every property its class
// grants, so an alias stops the extraction rather than being decided either way.
...
func buildPrefab(index *typeIndex, surface *logicSurface, resolver *modeResolver, isa *ISA,
	...
	dev := &device{class: class, state: asset.State, usedPower: asset.UsedPower}
	for i, slot := range asset.Slots {
		// A slot is addressed by its position in the list, so a roster that
		// numbered them any other way would shift every ls and ss operand.
		if slot.Index != i {
			return nil, fmt.Errorf("slot %d is listed at position %d", slot.Index, i)
		}
		name, ok := classNames[slot.Class]
		if !ok {
			return nil, fmt.Errorf("slot %d: Slot.Class has no member %d", slot.Index, slot.Class)
		}
...
//
// The enums are wide -- 358 properties -- and a class names a handful of them.
// Every selector a class never mentions takes the same path through the same
// bodies, so it is decided once and reused. That is a shortcut in cost only: a
// selector reaches an arm of its own exactly by being named, and a class that
// reaches selectors by arithmetic instead is excluded from the shortcut.
