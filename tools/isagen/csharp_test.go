...
// TestCSharpDoubleFidelity pins the widened float literals and derived constants,
// which must match the game bit for bit rather than be recomputed at full
// double precision.
...
// TestOperandKindTablesAgree keeps the token, JSON and Go name tables in step; a
// kind missing from any of them drops operands silently.
func TestOperandKindTablesAgree(t *testing.T) {
	...
	for token, kind := range helpTokenKinds {
		...
		if _, ok := goOperandKinds[kind]; !ok {
			t.Errorf("kind %q (token %q) has no generated constant", kind, token)
		}
	}
