...
// what its members say. The decompiler writes one file per top-level type
// under a directory per namespace component, so a type's qualified name is
// its path. Its declared bases are kept as written and resolved on demand.
...
// the tree declares no such type. An error means the file is there and could
// not be read as one type declaration; it is never reported as an absent
// type, since a class read as absent silently drops every property it
// declares from every prefab below it.
...
// empty string otherwise. A base the tree declares nowhere is a type from
// another assembly, where an inheritance chain legitimately ends; one it does
// declare is a type this program failed to place, unknown rather than absent.
...
// comparing by bare name since the interesting names (ICircuitHolder,
// IMemory, ILogicAtmospheric) are unique across the assembly.
func (x *typeIndex) derivesFrom(t *csharpType, name string) (tri, error) {
	...
	walk = func(t *csharpType) (tri, error) {
		// A type contributes its own answer once and is then cut off,
		// terminating a cycling base list -- sound at the root, though not
		// at every node, since or has already folded an earlier triMaybe in
		// before a later path could find triNo here instead.
		if seen[t.Qualified] {
			return triNo, nil
		}
		...
		for _, written := range t.Bases {
			...
			if base == nil {
				// A base this program could not place yields triMaybe, not
				// triNo, since what it implements is unknown.
