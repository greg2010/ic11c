...
// Definite assignment.
//
// A local the optimizer keeps in a register has no storage to have been zeroed:
// chip state survives power loss, chip removal and reflashing, and nothing
// zeroes registers. Reading one before it is written is undefined in C, and the
// consequence here is worse than a wrong number. IR generation gives the read no
// value, mem2reg turns it into undef, and the optimizer is then entitled to
// fold anything reached through it — a comparison against undef folds to
// whichever answer is convenient, and the device stores the comparison guarded
// are deleted. The emitted program does nothing and says nothing.
//
// The rule is therefore definite assignment, checked here: a tracked local must
// be written on every path reaching a read of it. It is deliberately a rule
// rather than a discovery, so that whether a program compiles does not depend on
// how well the analysis reasons. A value written on the previous iteration of a
// loop does not count, because the first iteration has not had one.
//
// Only the locals that live in registers are tracked. A global, an array, and a
// local whose address is taken live in the data region, which the entry
// prologue zeroes in one instruction, so those are zero on their first read and
// nothing here concerns them.

// checkDefiniteAssignment reports every read of a tracked local that is not
// assigned on all paths reaching it.
//
// It runs over the whole file after checking, and only when checking found no
// error: it reads Uses to recognize a variable, and a name that did not resolve
// is absent from it, which would make every mention of that name look like a
...
// trackedLocals collects the locals of one definition that live in a register,
// which are the ones nothing zeroes.
//
// Whether the address of a local was taken is read off the symbol rather than
// walked for here: checking has already seen every '&' in the file, which the
// walk below could not rely on — the address of a local can be taken above the
...
// registerClass reports whether a symbol is a local the optimizer can hold in a
// register. A const object is excluded for a different reason from the rest: it
// carries an initializer by the language's own rule, so no read of one can
// precede a write.
...
// declaredLocal finds the symbol a local declaration introduced, and nil for a
// declaration nothing reads.
func (a *assigner) declaredLocal(d *ast.VarDecl) *Symbol { return a.decls[d] }

// symbolOf names the object an lvalue designates, or nil for one that is not a
// variable.
...
//
// jumps holds one frame per enclosing loop or switch, since a break leaves the
// innermost of either and a continue the innermost loop. What the frame collects
// is the state at each jump, which is what the statement's exit merges.
