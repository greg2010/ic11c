 ...
 * different operand rule: a double target takes a long long right operand and
 ...
 * bool is a written conversion where a narrowing to long long would be, and it
...
constexpr long long kGainSteps = 3;
constexpr long long kDialModulus = 360;
constexpr long long kNibble = 0xF;
constexpr long long kFieldShift = 4;
...
void main(void) {
    while (true) {
        ...
        dial += (long long)reading;
        dial %= kDialModulus;

        long long field = (long long)reading;
        field &= kNibble;
