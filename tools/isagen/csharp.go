...
// literal, or reports i itself for ordinary code. Brace and paren matching
// relies on this so punctuation inside literals never affects nesting depth.
...
// only the layout whitespace before it. Requiring the block to be the very
// next thing keeps a declaration written without one from being read as the
// block belonging to whatever follows it.
...
// source order. It is the sibling of parseListInitializer for a table the
// game writes as one class per entry rather than as enum members.
...
// mapping each integer label to the type it constructs; the discard arm is
// skipped. Arms are matched rather than split apart, since the `=>` of an
// arm carries a bracket the top level splitter counts as nesting.
