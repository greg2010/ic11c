...
// the compiler accepts mean something else to the editor reading it. MicroC's
// short spelling names the same type, and a Windows toolchain reads a C long
// as 32 bits, so it is not written here. The constants sit on C's own typing
// boundaries: a constant takes the narrowest type that represents it rather
// than the type of the object it initializes, a hexadecimal constant may land
// on an unsigned type where a decimal one may not, and a bare 1 is an int, so
// it shifts 30 places and no further: `1 << 31` is refused because the answer
// does not fit the int C computes it in. The widening casts are what make each
// constant mean what it reads as.
//
