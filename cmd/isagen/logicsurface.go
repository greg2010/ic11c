...
// tri is a three-valued answer to "can this device do this".
//
// triMaybe is not a shrug. The game decides a handful of properties from live
// state -- what a logic transmitter is currently pointed at, whether a pipe
// connection is made -- and no static reading of the assembly can settle those.
// Reporting one of those as a plain yes or no would put a wrong answer into a
// table whose whole purpose is to catch wrong answers, so the uncertainty is
// carried through to the artifact instead.
type tri uint8
...
// flow is how a statement list ended, which decides what runs after it.
//
// A break is a distinct outcome from running off the end. The two continue in
// different places: a break leaves the enclosing switch, and a list that runs
// out continues with the statements that follow it. Reading them alike answers
// `case X: if (!HasFoo) break; return true;` with the return, which is the
// statement the game reaches only when the flag is set.
type flow uint8
...
// String names the outcome the way a diagnostic about a body reads.
func (f flow) String() string {
	switch f {
	case flowFell:
		return "runs past its end"
	case flowAnswered:
		return "answers"
	case flowBroke:
		return "leaves the switch"
	default:
		return "flow(" + strconv.FormatUint(uint64(f), 10) + ")"
	}
}
...
func (k surfaceKind) String() string {
	switch k {
	case kindReadLogic:
		return "CanLogicRead(LogicType)"
	case kindWriteLogic:
		return "CanLogicWrite(LogicType)"
	case kindReadSlot:
		return "CanLogicRead(LogicSlotType, int)"
	case kindWriteSlot:
		return "CanLogicWrite(LogicSlotType, int)"
	}
	return "surfaceKind(" + strconv.FormatUint(uint64(k), 10) + ")"
}
...
// surfaceProbes recognize a declaration of each method without modelling it, so
// that a class declaring one in a spelling [surfaceSignatures] does not reach --
// an expression body, an explicit interface implementation, a parameter list
// with something else in it -- is noticed rather than walked past. Walking past
// answers the question with the base's body, and the body a class overrode is
// the one body whose answer is certainly not the class's.
//
// The parameter type is what separates the whole-device form from the slot
// form, so a class declaring one of the two and not the other is the ordinary
// case rather than a miss. It is matched inside the parameter list to admit the
// qualified spelling of the enum as well as the bare one.
...
// group of switch cases as, capturing the selector, the offset subtracted from
// it and the bound.
//
// The compiler turns `case A: case B:` on adjacent enum values into one
// unsigned comparison, and the decompiler cannot see the switch behind it. The
// two forms mean the same thing and both have to be read, because between them
// they carry most of what a device exposes: solar panels reach Horizontal and
// Vertical only through this shape.
var selectorOffsetRE = regexp.MustCompile(`^(\w+) - (\d+)$`)

// device is everything about one prefab the logic surface is a function of.
type device struct {
	class *csharpType
	// state holds the eleven flags Device switches on, by their game names.
...
//
// Every body it meets is a switch over the selector, an equality test against
// it, or a delegation upward, over a vocabulary of serialized fields and class
// facts. Anything outside that vocabulary evaluates to triMaybe rather than to
// a guess.
type logicSurface struct {
	...
	// mentioned caches, per class and selector form, the selectors any body in
	// the inheritance chain names. A selector named nowhere reaches the same
	// default arm as every other unnamed one, which is what keeps this from
	// evaluating hundreds of identical paths per prefab. A nil entry means the
	// class decides selectors by arithmetic and nothing can be skipped.
...
// unusedSelectorValue picks that number, which has to sit inside the range the
// bodies compare at rather than beyond it.
//
// Beyond it is not outside. A folded group of switch cases is written as
// `selector - offset <= bound` and evaluated in the enum's own unsigned
// underlying type, so a value above that type's range is masked back into it and
// stands for whichever member it lands on; a representative of 1<<20 against a
// ushort selector is numerically zero in every one of those tests. The largest
// value the type holds is inside the range, outside both enums, and stays
// outside after a subtraction wraps it.
...
// names. A selector absent from it cannot change which path any of those bodies
// takes, and so reaches the same answer as every other unnamed one.
//
// Both directions are collected together and the whole inheritance chain is
// walked, not just the chain of one method: a class overriding one direction
// can delegate to the other, and the game does exactly that in places.
//
// A nil result means no selector can be skipped. A body that tests the selector
// by arithmetic reaches a group of them without naming any, so for a class with
// one there is nothing the name alone rules out.
...
// parameter is one the member scan above can see: the subject of a switch, an
// operand of an equality test, or the argument handed to the implementation
// being overridden.
//
// Recognising the shape rather than a spelling is the whole point. The
// decompiler writes a folded group of switch cases as `selector - 20 <= X`,
// which reaches twenty selectors while naming two; a body that bound the
// selector to a local, offset it by a different operator, or spaced the
// subtraction differently would reach a group just as quietly. What the shortcut
// answers wrongly it answers with no diagnostic, because a device refusing a
// property is a legitimate answer, so an arrangement this does not recognise
// disables the shortcut rather than being read as name comparison. That costs a
// slower run and decides the same answers.
...
// the end of what the assembly declares.
//
// A chain that ends at a name the tree holds nothing under has left the
// assembly, and nothing outside it declares a logic surface. One that stopped
// at a base the tree does declare stopped early, and a class read as declaring
// no surface on that basis would be reported as refusing every property it has.
//
// Exhausting the depth bound is an error rather than a resolved chain, as it is
// in [logicSurface.method]: both callers turn a resolved chain into a denial,
// and a chain this program cannot walk to its end says nothing about what the
// classes above the point it stopped declare.
...
//
// A class the strict signature misses and the probe finds is an error rather
// than a class declaring nothing: both callers walk on to the base when a class
// declares nothing, and there they would read an implementation this class
// replaced. A denial from the wrong body is the failure the whole table exists
// to prevent, so the shape is reported for a human to model.
...
// which is a class with no logic surface where the chain ran out at the edge of
// the assembly. Which of the two it was is [logicSurface.chainResolves].
//
// Exhausting the depth bound is an error rather than a nil method: nil is read
// as a denial where the chain resolved, and a chain this program cannot walk to
// its end says nothing about what the class declares above the point it
// stopped.
...
// ended.
//
// It falls through to the statements after the switch when no arm and no
// default applies, and when the arm that does apply breaks instead of
// answering. It answers, undecided, on a switch over anything but the selector
// and on one whose applicable arm cannot be told: neither says what the
// statements after the switch would say either.
...
// labelMatches reports whether a case label names the selector under query, and
// whether the label could be read at all.
//
// A label this program cannot read is not a label that fails to match. Every
// later arm and the default one are reached only by not matching this one, so
// answering a shape outside the vocabulary with a non-match sends the question
// to an arm the game may never take, and reports a device as refusing a
// property it accepts.
//
// The decompiler writes a group of adjacent members as a disjunction of
// patterns, which is read here. A relational or negated pattern is not.
...
// evalBaseCall continues the question at the class above the one that declared
// the body being evaluated.
//
// The called method decides the direction, which is not always the direction of
// the question: the game has classes whose CanLogicWrite delegates to
// base.CanLogicRead, and following the question's own direction there would
// report a writable property the game refuses, or the reverse.
//
// The arguments are checked too. A delegation that substitutes another selector
// or another slot asks a different question, and following it would answer the
// wrong one.
...
// constantBool answers a class-level bool property whose overrides are literal.
//
// A class that declares the property in any other form yields triMaybe: the
// answer is real but is not a constant of the class, and reading past the
// declaration to the inherited literal would report the wrong one.
...
// declaredBool returns the value a class declares a bool member as, and whether
// it declares it at all.
//
// A declaration in a form other than a literal expression body still counts as
// one: the class has an answer of its own, the inherited literal above it is not
// that answer, and reading past it would report the wrong one.
...
const (
	...
	// the number those tables give it. A reference the tables hold no member
	// under is not one of these: the bodies subtract and compare those numbers,
	// and a miss read as zero would decide every selector against a bound the
	// body never wrote.
	valEnumMember
	// valSlotClassMember is a member of Slot.Class, which a slot's class is
	// compared against by name. A reference the enum declares no member under is
	// not one of these: the roster names a slot's class with a member of that
	// enum, so a name absent from it matches nothing, and reading the miss as a
	// non-match denies the property on every slot the body grants it to.
	valSlotClassMember
...
// value classifies one side of a comparison.
//
// An expression outside the vocabulary is unknown rather than an error, which
// leaves the comparison undecided. A literal this program matched and then could
// not read is an error: it is a bound the body does write, and dropping it to
// unknown hides a number the game compares against.
...
// actually form are decided; anything else is left undecided rather than
// guessed at.
//
// Each pair is written once and tried in both orders, since which side a body
// puts the constant on is a matter of style.
