...
// fixtureTypes indexes the hand-written stand-in for the decompiled game.
func fixtureTypes(t *testing.T) *typeIndex {
	t.Helper()
...
// perturbedTypes indexes a copy of that stand-in with the named files written
// over it.
//
// The copy is what lets a body shape be stated one test at a time: the stand-in
// itself is the input every golden in this package is compared against, and a
// shape added to it would move all of them.
func perturbedTypes(t *testing.T, files map[string]string) *typeIndex {
	t.Helper()
	root := t.TempDir()
	if err := os.CopyFS(root, os.DirFS(filepath.Join("testdata", "decompiled"))); err != nil {
		t.Fatalf("copy the decompiled fixture: %v", err)
	}
...
func fixtureType(t *testing.T, qualified string) *csharpType {
	t.Helper()
	found, err := fixtureTypes(t).lookup(qualified)
	if err != nil {
...
func TestParseCsharpType(t *testing.T) {
	tests := []struct {
		...
		{
			// A struct's base list holds only interfaces, so an entry of one
			// resolving here is never the base class.
			name:      "struct",
			declared:  "Grid",
		...
		{
			// The colon in a constraint opens no base list, and reading it as one
			// gives the type a base the declaration does not name.
...
// TestResolutionCandidates pins the order C# itself searches, which is what
// lets a base written with no namespace resolve to the right one of the several
// types the game declares under that name.
...
// TestBaseClass covers the one thing the base list does not say: which of its
// entries is the base class. Only resolving them tells the two apart.
func TestBaseClass(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.class, func(t *testing.T) {
			...
			base, err := index.baseClass(class)
			if err != nil {
...
// TestDerivesFrom states what the walk makes of each shape a base list takes,
// and in particular the two that resolve to nothing for opposite reasons: a
// name the tree holds nothing under is a type from another assembly and settles
// the answer, and one it does hold leaves it open.
func TestDerivesFrom(t *testing.T) {
	// The tree declares Housing under Assets.Scripts.Objects.Electrical, which
...
`
	...
	// tree declares and this file cannot reach, so Open answers triMaybe. The
	// walk visits Open once and answers the second path triNo, which the first
	// path's triMaybe has to survive.
...
`
	// A cycle, which a base list this program misparsed can produce and which
	// the walk has to answer rather than follow forever. Pong also names a base
	// it cannot place, so the answer that escapes the cycle is triMaybe.
...
`
	...
	tests := []struct {
		name string
		// files are written over a copy of the fixture tree, leaving the
		// fixture itself the input every golden in this package rests on.
		...
		{name: "base class", class: "Assets.Scripts.Objects.Electrical.Housing", want: "Thing", is: triYes},
		{name: "interface", class: "Assets.Scripts.Objects.Electrical.Housing", want: "ICircuitHolder", is: triYes},
		{name: "unrelated interface", class: "Assets.Scripts.Objects.Pipes.Sensor", want: "ICircuitHolder", is: triNo},
		{name: "unknown name", class: "Assets.Scripts.Objects.Thing", want: "INotDeclared", is: triNo},
		{
		...
		{
			// The chain above an unplaceable base is unread whatever is asked of
			// it, so a name it could hold anywhere is open too.
		...
		{
			// The same shape asked a name the shared type's own base list holds,
			// which settles on the first path and must not be reopened by the
			// second.
...
// outcome rather than an error: every inheritance chain ends in one.
func TestLookupAbsentType(t *testing.T) {
	index := fixtureTypes(t)
	found, err := index.lookup("UnityEngine.MonoBehaviour")
	if err != nil {
...
func TestMemberBody(t *testing.T) {
	device := fixtureType(t, "Assets.Scripts.Objects.Pipes.Device")
	decl, _, ok := findMember(device, surfaceSignatures[kindReadLogic])
	if !ok {
		t.Fatal("Device declares no CanLogicRead(LogicType)")
	}
	body, err := memberBody(decl)
	if err != nil {
		t.Fatalf("memberBody: %v", err)
	}
	if got := len(splitExprList(body)); got == 0 {
		t.Error("member body is empty")
	}
