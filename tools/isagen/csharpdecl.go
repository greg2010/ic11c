...
// braces on their own lines and no preprocessor directives, so brace and
// semicolon counting is enough; anything the scan cannot account for is an
...
// walkDecls appends a record for every declaration under body, descending
// into nested types. prefix is the path of the declaration body belongs to,
// empty at the top of a type. An enum is recorded whole and not descended
// into, since records sorted by name would hide a reordering of its ordinals.
...
// against the name the caller expected. The decompiler writes one file per
// top-level type, so a file holding two is a layout change reported rather
// than half read.
