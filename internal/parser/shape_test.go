...
// The shape functions render a tree as a fully parenthesized prefix form. Every
// node contributes its own parentheses, so grouping is explicit and a
// precedence defect cannot hide behind reassociated output the way it can in a
// source round trip. They also assert that every node they visit carries a
// valid position, which is how the position requirement gets exercised across
// the whole testdata corpus.
...
func declShape(t *testing.T, d ast.Decl) string {
	...
	case *ast.VarDecl:
		...
		return sexpr(label, sexpr(x.Name, fields...))
	case *ast.BadDecl:
		return "(baddecl)"
	default:
