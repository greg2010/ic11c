...
// Thing.ModeStrings names the settings a device's Mode property selects
// between, and its length is how far that property counts. A program that
// computes a mode number has nothing else to check it against: the chip accepts
// any number and the device clamps it, so a write past the end silently lands
// somewhere else.
//
// The property is virtual with a two element default, and the overrides reach
// their strings through several indirections: a literal array, a static field
// holding one, the names of a C# enum, or an EnumCollection wrapping an enum.
// Everything below resolves those four and reports anything else unresolved
// rather than substituting the inherited default, which would understate a
// device's range.

// modeStringsRE matches the property declaration on a member's raw text, which
// is where both the expression-bodied and the initialized-auto-property forms
// keep their value.
...
// rendersProper reads an EnumCollection constructor's argument list for the one
// argument that decides whether the names are rendered into labels, and reports
// whether the list said.
//
// The parameter defaults to true, so an empty list renders. Any other list is a
// shape this program does not read: the flag may be there positionally, or
// beside arguments that are not modelled here, and either reading names half the
// modes wrong.
...
// gameStringCollection resolves the names a localized collection field exposes.
//
// They are the enum's own, whatever the constructor's toProper argument says.
// That argument renders the display strings the collection carries beside the
// names, and a mode list reads the names; EnumCollection properizes the names
// themselves, which is why the two are resolved apart rather than together.
...
// enumNames returns an enum's member names in the order .NET reflection reports
// them, which is by value and is the order a mode number indexes.
//
// A member whose value repeats another's, or a set that does not run from zero,
// would make the index and the value disagree, and the resolution stops rather
// than emit a table where a mode number names the wrong setting.
...
// it.
//
// A file that declares no enum under the name is not an error: the reference
// resolved to a type that is not one, which is the same unresolved answer as a
// mode list the game fills in at run time. Any other failure is a declaration
// this program could not read, and reporting that as unresolved too would hide
// the one case a reader has to act on.
