...
// TestRenderPreludeOpensWithTheToolDirectives pins both directives to the top and
// in that order: linters read the generated marker as the first line, and
// clang-format holds its own directive to end of file.
...
func TestRenderCompileFlagsIsAnArgumentPerLine(t *testing.T) {
	tests := []struct {
		...
		{name: "elsewhere in the tree", header: "../../ic10/generated/" + preludeFileName},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
func TestRenderFlagsForNamesTheHeaderRelativeToTheFlagsFile(t *testing.T) {
	tests := []struct {
		...
		{name: "beside the corpus", flags: defaultFixtureFlagsPath, want: "../../ic10/generated/" + preludeFileName},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
