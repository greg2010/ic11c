...
// this is the one corpus program that asks for more than that. Five values end
// up in the data region as a result: a slot each, a poke to put one there and a
// get to fetch it back, and a get costs a line every time the value is read.
// Reading a section at a time and folding it before moving on would need none
// of them, and is the shape to write where the line budget is tight.
//
// Nothing is written back to a section: a gas sensor answers readings and
// accepts none.
...
void main(void) {
    while (true) {
        ...
        __ic_store(display, On, fouled > kFouledCeiling || lowestPressure < kMinKilopascals);
        __ic_yield();
    }
}
